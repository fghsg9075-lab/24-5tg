...
const App: React.FC = () => {
  ...
  const handleLogin = (user: User) => {
    ...
    localStorage.setItem('nst_has_seen_welcome', 'true');
    setState(prev => ({ ...prev, user, view: 'STUDENT_DASHBOARD' as any, selectedBoard: user.board || null, selectedClass: user.classLevel || null, selectedStream: user.stream || null, language: user.board === 'BSEB' ? 'Hindi' : 'English', showWelcome: false }));
  };
  ...
  const handleContentGeneration = async (type: ContentType, count?: number) => {
    ...
    if (onlineContent) {
        if (type === 'PDF_FREE' && onlineContent.freeLink) {
            ...
            onlineContent = { ...onlineContent, content: onlineContent.ultraPdfLink, type, price: onlineContent.price !== undefined ? onlineContent.price : 10 };
            foundAdminContent = true;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white font-sans relative">
      ...
      {!state.originalAdmin && state.user?.role === 'ADMIN' && state.view !== 'ADMIN_DASHBOARD' && (
          <div className="fixed bottom-20 right-6 z-[90]">
              <button 
                  onClick={() => setState(prev => ({...prev, view: 'ADMIN_DASHBOARD'}))} 
                  className="bg-slate-900 text-white font-bold py-3 px-6 rounded-full shadow-2xl flex items-center gap-2 border-4 border-white transform hover:scale-105 transition-all"
              >
                  <BrainCircuit size={20} /> Admin Console
              </button>
          </div>
      )}
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto p-4 mb-8">
        <ErrorBoundary onReset={() => setState(prev => ({...prev, view: 'CHAPTERS', lessonContent: null }))}>
            {!state.user ? (
                ...
                <>
                    ...
                    {activeWeeklyTest ? (
                        ...
                        state.view === 'STUDENT_DASHBOARD' as any && <StudentDashboard user={state.user} dailyStudySeconds={dailyStudySeconds} onSubjectSelect={handleSubjectSelect} onRedeemSuccess={u => setState(prev => ({...prev, user: u}))} settings={state.settings} onStartWeeklyTest={handleStartWeeklyTest} isAdmin={state.user.role === 'ADMIN'} onAdminAction={(action) => { if(action === 'OPEN_CONSOLE') setState(prev => ({...prev, view: 'ADMIN_DASHBOARD'})) }} />
                    )}
                    ...
                    {state.view === 'CHAPTERS' && state.selectedSubject && <ChapterSelection chapters={state.chapters} subject={state.selectedSubject} classLevel={state.selectedClass!} loading={state.loading && state.view === 'CHAPTERS'} user={state.user} onSelect={onChapterClick} onBack={goBack} stream={state.selectedStream} board={state.selectedBoard}/>}
                    {state.view === 'LESSON' && state.lessonContent && <LessonView content={state.lessonContent} subject={state.selectedSubject!} classLevel={state.selectedClass!} chapter={state.selectedChapter!} loading={false} onBack={goBack} isAdmin={state.user.role === 'ADMIN'} onEditContent={() => { /* Handled in LessonView or pass modal state up? */ }} />}
                </>
            )}
        </ErrorBoundary>
      </main>
