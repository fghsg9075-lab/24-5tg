...
export const AdminDashboard: React.FC<Props> = ({ onNavigate, settings, onUpdateSettings, onImpersonate, logActivity }) => {
  ...
  // --- BULK UPLOAD STATE ---
  const [bulkData, setBulkData] = useState<Record<string, {free: string, ultra: string, price: number}>>({});

  // --- EDITING STATE ---
  const [editingChapterId, setEditingChapterId] = useState<string | null>(null);
  const [editConfig, setEditConfig] = useState<ContentConfig>({ freeLink: '', premiumLink: '', ultraPdfLink: '', price: 0 });
...
<html lang="en">
          ...
          if (activeTab === 'BULK_UPLOAD') {
              ...
              ch.forEach(c => {
                  ...
                  if (stored) {
                      ...
                      tempBulk[c.id] = { free: d.freeLink || '', ultra: d.ultraPdfLink || '', price: d.price || 10 };
                  } else {
                      tempBulk[c.id] = { free: '', ultra: '', price: 10 };
                  }
              });
  ...
  const loadChapterContent = async (chId: string) => {
      ...
      if (stored) {
          ...
          // Default State
          setEditConfig({ freeLink: '', premiumLink: '', ultraPdfLink: '', price: 5 });
          setEditingMcqs([]);
          setEditingTestMcqs([]);
          setVideoPlaylist([]);
      }
      ...
      {activeTab === 'DASHBOARD' && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 mb-6 animate-in fade-in">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                  ...
                  <DashboardCard icon={LayersIcon} label="Ultra PDF" onClick={() => setActiveTab('BULK_UPLOAD')} color="red" />
      ...
      {activeTab === 'BULK_UPLOAD' && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 animate-in slide-in-from-right">
              <div className="flex items-center gap-4 mb-6 border-b pb-4">
                  ...
                  <h3 className="text-xl font-black text-slate-800">Ultra PDF Manager</h3>
              </div>
              <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 mb-6 text-sm text-indigo-800">
                  <strong>Instructions:</strong> Upload "Ultra PDF" Links here. These are available to Ultra Subscribers or via Credit Purchase.
              </div>
              ...
              {selSubject && selChapters.length > 0 && (
                  <div className="space-y-4">
                      <div className="flex justify-between items-center bg-slate-100 p-3 rounded-lg font-bold text-slate-600 text-xs uppercase">
                          ...
                          <div className="w-1/3">Ultra PDF Link</div>
                          <div className="w-16">Cost</div>
                      </div>
                      
                      {selChapters.map((ch, idx) => {
                          const data = bulkData[ch.id] || { free: '', ultra: '', price: 10 };
                          return (
                              <div key={ch.id} className="flex gap-2 items-center">
                                  ...
                                  <div className="w-1/3">
                                      <input 
                                          type="text" 
                                          placeholder="Paste Ultra PDF Link..." 
                                          value={data.ultra}
                                          onChange={e => setBulkData({...bulkData, [ch.id]: { ...data, ultra: e.target.value }})}
                                          className="w-full p-2 border border-indigo-200 bg-indigo-50 rounded text-xs focus:ring-1 focus:ring-indigo-500 outline-none"
                                      />
                                  </div>
                                  <div className="w-16">
                                      <input 
                      ...
                      {activeTab === 'CONTENT_PDF' && (
                          <div className="space-y-6">
                              ...
                              <div>
                                  ...
                                  <div className="flex items-center bg-white border rounded-xl overflow-hidden">
                                      <div className="bg-slate-100 p-3"><Link size={16} className="text-indigo-400" /></div>
