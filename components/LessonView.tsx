...
import { LessonContent, Subject, ClassLevel, Chapter, MCQItem, ContentType, Board, Stream } from '../types';
import { ArrowLeft, Clock, AlertTriangle, ExternalLink, CheckCircle, XCircle, Trophy, BookOpen, Play, Edit2 } from 'lucide-react';
...
export const LessonView: React.FC<Props> = ({ 
  ...
  // --- MCQ RENDERER ---
  if ((content.type === 'MCQ_ANALYSIS' || content.type === 'MCQ_SIMPLE') && Array.isArray(content.mcqData) && content.mcqData.length > 0) {
      const score = Object.keys(mcqState).reduce((acc, key) => {
          const qIdx = parseInt(key);
          const q = content.mcqData?.[qIdx];
          return acc + (q && mcqState[qIdx] === q.correctAnswer ? 1 : 0);
      }, 0);
      ...
      return (
          <div className="flex flex-col h-full bg-slate-50 animate-in fade-in">
               ...
               <div className="flex items-center justify-between p-4 bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                   ...
                   {isAdmin && (
                       <button onClick={() => setShowEditor(true)} className="flex items-center gap-2 bg-blue-100 text-blue-700 px-3 py-2 rounded-lg font-bold text-xs">
  ...
  const isVideo = (content.type === 'PDF_VIEWER' || content.type === 'VIDEO_LECTURE') && 
                  (content.content?.includes('youtube.com') || content.content?.includes('youtu.be') || content.content?.includes('drive.google.com/file') || content.content?.includes('.mp4') || (Array.isArray(content.videoPlaylist) && content.videoPlaylist.length > 0));

  if (isVideo) {
      const [currentVideoIndex, setCurrentVideoIndex] = useState(0);
      const playlist = (Array.isArray(content.videoPlaylist) && content.videoPlaylist.length > 0) 
          ? content.videoPlaylist 
          : [{ title: chapter.title, url: content.content || '' }];
      
      // Safe Index Access
      const safeIndex = (currentVideoIndex >= 0 && currentVideoIndex < playlist.length) ? currentVideoIndex : 0;
      const currentVideo = playlist[safeIndex] || { title: "Error", url: "" };
      ...
      try {
          if (embedUrl.includes('youtube.com/watch')) {
              ...
              if (parts.length > 1) {
                  const videoId = parts[1].split('?')[0]; // Remove query params if any
      ...
      return (
          <div className="flex flex-col h-[calc(100vh-80px)] bg-slate-900">
              ...
              <div className="flex items-center justify-between p-3 bg-slate-800 border-b border-slate-700 shadow-sm">
                   ...
                   {isAdmin ? (
                       <button onClick={() => setShowEditor(true)} className="flex items-center gap-2 bg-slate-700 text-white px-3 py-1 rounded-lg font-bold text-xs hover:bg-slate-600">
                           <Edit2 size={14} /> Edit
                       </button>
  ...
  if (content.type === 'PDF_VIEWER' || content.type === 'PDF_FREE' || content.type === 'PDF_PREMIUM' || content.type === 'PDF_ULTRA') {
      ...
      const isPdf = contentUrl.toLowerCase().endsWith('.pdf') || contentUrl.includes('drive.google.com') || contentUrl.includes('docs.google.com');
      ...
      return (
          <div className="flex flex-col h-[calc(100vh-80px)] bg-slate-100">
              ...
              <div className="flex items-center justify-between p-3 bg-white border-b border-slate-200 shadow-sm">
                   ...
                   {isAdmin ? (
                       <button onClick={() => setShowEditor(true)} className="flex items-center gap-2 bg-blue-100 text-blue-700 px-3 py-1 rounded-lg font-bold text-xs hover:bg-blue-200">
                           <Edit2 size={14} /> Edit
                       </button>
                   ) : <div className="w-10"></div>}
              </div>
      ...
      return (
        <div className="bg-white min-h-screen pb-20 animate-in fade-in">
           ...
           <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-slate-100 px-4 py-3 flex items-center justify-between shadow-sm">
               ...
               {isAdmin ? (
                   <button onClick={() => setShowEditor(true)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-full">
                       <Edit2 size={20} />
                   </button>
  ...
  // --- NOTES (MARKDOWN) RENDERER (FALLBACK) ---
  return (
    <div className="bg-white min-h-screen pb-20 animate-in fade-in">
       {showEditor && (
           <ContentEditorModal 
       ...
       <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-slate-100 px-4 py-3 flex items-center justify-between shadow-sm">
           ...
           <div className="text-center">
               <h3 className="font-bold text-slate-800 text-sm leading-tight">{chapter.title}</h3>
               <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">{content.subtitle || 'Study Notes'}</p>
           </div>
           {isAdmin ? (
               <button onClick={() => setShowEditor(true)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-full">
                   <Edit2 size={20} />
